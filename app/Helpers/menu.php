...
if (!function_exists('get_main_menu')) {
    ...
    function get_main_menu($menu_items, $detph = 1, $is_submenu = false, $parentId = null)
    {
        if ($is_submenu) {
            ...
            echo '<ul role="menu" class="sub-menu row ">';
        ...
        foreach ($menu_items as $k => $v) {
            ...
            $class = "menu-item menu-item-" . $v->item_id . ' ' . $class_current;
            if (isset($v->children)) {
                $class = "menu-item-has-children mega_menu" . $class_current;
            }
            ...
            if (!$is_submenu) {
                ...
                <li class="<?php echo esc_attr($class); ?>">
                    ...
                    <a href="<?php echo esc_attr($v->url); ?>" <?php echo $target; ?>>
                        <?php echo esc_attr($v->name); ?>
                    </a>
                    <?php

                    if (isset($v->children)) {
                        get_main_menu($v->children, 2, true, $v->id);
                    }
                    ?>
                </li>
                <?php
