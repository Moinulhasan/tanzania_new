    ...
    <div class="layout-top-spacing">
        <div class="statbox widget box box-shadow">
            ...
            <div class="widget-header">
                ...
                <form action="{{route('count-edit',$single->id)}}" method="post">
                    @csrf
                    @method('put')
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <select name="menu" id="menuName" class="form-control">
                                ...
                                @foreach($listMenus as $menu)
                                    <option value="{{$menu->id}}" {{$single->menu_id == $menu->id ?'selected':''}}> {{$menu->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mt-3 col-md-8">
                            <label for="count">Enter the Row Count</label>
                            <input type="number" value="{{$single->title}}" id="count" class="form-control" name="count" >
                        </div>
                        {{--                        <div class="col-md-8">--}}
                        {{--                            <div class="mt-3">--}}
