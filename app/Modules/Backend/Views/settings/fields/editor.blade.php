...
@endif
<div class="gmz-editor-media sdfsdfsdfsdf" data-toggle="modal" data-multi="true" data-target="#gmzMediaModal" data-url="{{dashboard_url('all-media')}}" data-name="{{$id}}"></div>
<div class="{{$classEditor}} gmz-field form-group {{$layout}} gmz-field-{{$type}} @if($translation == true) gmz-field-has-translation @endif" id="gmz-field-{{$id}}-wrapper" @if(!empty($condition)) data-condition="{{$condition}}" @endif {{ $classLangs }}>
    ...
    @foreach($langs as $key => $item)
        <div style="height: 100%;" class="{{get_lang_class($key, $item)}}" @if(!empty($item)) data-lang="{{$item}}" @endif>
            ...
            <textarea class="gmz-editor-content gmz-hidden-field" style="margin-bottom: 50px" name="{{$id}}{{get_lang_suffix($item)}}">{!! get_translate($value, $item) !!}</textarea>
            <textarea id="gmz-field-{{$id}}{{get_lang_suffix($item)}}" class="myeditorinstance" >{!! get_translate($value, $item) !!}</textarea>
        </div>
    @endforeach
    <style>
        .tox-promotion{
