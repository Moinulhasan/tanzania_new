@php
    $can_review = is_user_can_review('', $dest->id, 'travel_g');
    // dd($dest->id);
    $comments = get_comment_list($dest->id, [
    ...
    <div class="post-comment parent-form" id="gmz-comment-section">
        <div class="comment-form-wrapper">
            <form action="{{ url('add-comment') }}" class="comment-form form-sm gmz-form-action form-add-post-comment"
                ...
                <input type="hidden" name="post_id" value="{{ $dest->id }}"/>
                {{-- <input type="hidden" name="post_id" value=""/> --}}
                <input type="hidden" name="comment_id" value="0"/>
                <input type="hidden" name="comment_type" value="travel_g"/>
                <div class="row">
