    ...
    window.GmzOption = {
        ...
        initEditor: function(){
            ...
            function quill_img_handler(){
                ...
                var result = $(this);
                var list_identiry =  result[0]['quill']['container'];
                //list_identiry = $.parseHTML(list_identiry);
               

                list_identiry = $(list_identiry).attr('name');


                 $('.gmz-editor-media').each(function(){
                     if($(this).attr('data-name') == list_identiry){
                          $(this).trigger('click');
                     }
                 })
            ...
            //     var t = $(this);
            //     base.quill[t.attr('id')] = new Quill(this, {
            //         modules: {
            //             imageResize: {
            //                 displaySize: true
            //             },
        ...
        initListItems: function(){
                    ...
                    if (respon.status) {
                        if (respon.html !== '') {
                           ...
                           // parentItems.find('.card:last-child input[type="text"]').first().focus();
                            ...
                            // $(this).prev('.gmz-list-item card:nth-last-of-type(1)').addClass('sdfdsfdsf ff');

                            var qeditor =  parentItems.find('.card:last-child .gmz-quill-editor');
                            ...
                            base.quill[qeditor.attr('id')] = new Quill('.item_added', {
                                modules: {
                                    imageResize: {
                                        displaySize: true
                                    },
                                ...
                                placeholder: 'Compose an epicc...',
                                theme: 'snow',  // or 'bubble'
                            });

                           // parentItems.find('.card:last-child .ql-toolbar span:nth-of-type(2) .ql-bold').trigger('click');

                        

                            base.quill[qeditor.attr('id')].on('text-change', function (delta, oldDelta, source) {
                               
                                var content = qeditor.find('.ql-editor').html();
                                qeditor.parent().find('.gmz-editor-content').val(content).change();
                            });
            ...
            $(document).on('click', '.gmz-quill-editor', function(){
                var focused_item = $(this);

            
                base.quill[focused_item.attr('id')].focus();
                                ...
                                if(imageEditor.length){
                                    for(var i = 0; i < imageEditor.length; i++){
                                       // alert('dfdfdf');
                                        ...
                                        // base.quill['gmz-field-highlight[content][16662192356108][]'].insertEmbed(range.index, 'image', imageEditor[i]);
                                    }
                                }
                            }
                        }
